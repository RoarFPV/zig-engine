...
pub fn build(b: *Builder) void {
    ...
    const debug_build = b.option(bool, "debug", "Debug build") orelse false;

    //const mode = b.standardReleaseOptions();
    const exe = b.addExecutable(.{
        .name = "zngine",
        .root_source_file = .{ .path = "src/engine.zig" },
        .optimize = if (debug_build) std.builtin.Mode.Debug else std.builtin.Mode.ReleaseFast,
    });
    ...
    if (builtin.os.tag == .windows) {
        ...
        exe.addLibraryPath(b.fmt("{s}/lib/x64", .{sdl_path}));
        exe.addIncludePath(b.fmt("{s}/include", .{sdl_path}));
        exe.addIncludePath("C:\\Program Files (x86)\\Windows Kits\\10\\Include\\10.0.17763.0\\shared\\evntprov.h");
